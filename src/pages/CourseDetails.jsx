...
function CourseDetails() {
  ...
  const isEnrolled = (() => {
    if (!user || !studentsEnroled) return false
    const enrolledIds = studentsEnroled.map((s) => (s && s._id ? String(s._id) : String(s)))
    return enrolledIds.includes(String(user._id))
  })()

  const handleBuyCourse = () => {
    if (token) {
      // prevent purchasing if already enrolled
      if (isEnrolled) {
