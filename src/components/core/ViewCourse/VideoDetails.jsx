...
const VideoDetails = () => {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (!courseId && !sectionId && !subSectionId) {
        ...
        if (!filteredVideoData || !filteredVideoData.length) {
          setVideoData(null)
  ...
  const goToNextVideo = () => {
    ...
    if (currentSubSectionIndx !== noOfSubsections - 1) {
      const nextSubSectionId = // find next video of same section
  ...
  const goToPrevVideo = () => {
    ...
    if (currentSubSectionIndx !== 0) {
      const prevSubSectionId = // find video same section ki previous video
        courseSectionData[currentSectionIndx]?.subSection?.[
          currentSubSectionIndx - 1
        ]?._id
      ...
      if (prevSubSectionLength === 0) return
              ...
              <IconBtn
                ...
                onclick={() => {
                  if (playerRef?.current) {
                    ...
                    playerRef?.current?.seek(0)//seek(0)=>start from initial
                    setVideoEnded(false)
                  }
                }}
                text="Rewatch"
